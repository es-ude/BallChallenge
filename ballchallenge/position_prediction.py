...
from ballchallenge.accelerometer_dataset import make_functions_for_normalizing_and_denormalizing_positions, \
    AccelerometerDatasetWithPointLabels
from ballchallenge.training import LossMetric, create_train_steps_factory, create_test_steps_factory

from ballchallenge.data_loading import get_input_data_shape, DATASET_ROOT, downsample_and_substract_mean, \
    split_dataset, DEVICE
...
def create_position_model(input_shape: tuple[int, int], kernel_sizes: Sequence[int] = (26, 26, 26), out_channels: Sequence[int]=(3, 3, 3), lin_act: Callable[[], nn.Module] = nn.Hardsigmoid, conv=bnormed_conv, linear_out_features: Sequence[int]=(2,)) -> nn.Module:
    ...
    all_layers = tuple(itertools.chain(convs, linears))
